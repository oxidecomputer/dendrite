...
//! Types from API version 4 (MCAST_SOURCE_FILTER_ANY) that changed in
//! version 5 (MCAST_STRICT_UNDERLAY).
//!
//! Changes in v5:
...
/// A validated admin-local IPv6 multicast address (API version 4).
///
/// In v4, admin-local addresses are validated against ff04::/16 (scope 4).
/// In v5+, this was renamed to `UnderlayMulticastIpv6` and tightened to
...
/// Convert from API v5 response to v4 response.
...
/// Response structure for external multicast group operations (API version 4).
#[derive(Debug, Deserialize, Serialize, JsonSchema)]
...
/// (API version 4).
///
/// Tags are optional in v4 for backward compatibility. If not provided,
/// the existing tag is preserved.
#[derive(Debug, Deserialize, Serialize, JsonSchema)]
pub struct MulticastGroupUpdateUnderlayEntry {
    /// Tag for validating update requests. Optional in v4. If not provided,
...
/// (API version 4).
///
/// Tag validation is optional in v4 for backward compatibility.
#[derive(Debug, Deserialize, Serialize, JsonSchema)]
...
/// Path parameter for underlay multicast group endpoints (API version 4).
///
...
/// Request body for creating underlay multicast groups (API version 4).
///
