...
pub enum Route {
    ...
    List {
        ...
        /// router whose table to operate on (0 = default)
...
async fn route_list(
    ...
    if v4 {
        let routes: Vec<types::Ipv4Routes> = client
            .router_route_ipv4_list_stream(router_id, None)
            .try_collect()
            .await
    ...
    if v6 {
        let routes: Vec<types::Ipv6Routes> = client
            .router_route_ipv6_list_stream(router_id, None)
            .try_collect()
            .await
...
async fn route_get(
    ...
    match &cidr {
        IpNet::V4(c) => {
            let targets = client
                .router_route_ipv4_get(router_id, c)
                .await
        ...
        IpNet::V6(c) => {
            let targets = client
                .router_route_ipv6_get(router_id, c)
                .await
...
async fn route_del(
    ...
    match cidr {
        IpNet::V4(c) => match gw {
            ...
            None => client
                .router_route_ipv4_delete(router_id, &c)
                .await
        ...
        IpNet::V6(c) => match gw {
            ...
            None => client
                .router_route_ipv6_delete(router_id, &c)
                .await
...
pub async fn route_cmd(client: &Client, cmd: Route) -> anyhow::Result<()> {
    match cmd {
        Route::List { family, router_id } => {
            route_list(client, router_id, family).await
        }
        Route::Get { cidr, router_id } => {
            route_get(client, router_id, cidr).await
        }
        Route::Add { cidr, link_path, gw, vlan_id, router_id } => {
            route_add(client, router_id, cidr, link_path, gw, vlan_id).await
        }
        Route::Del { cidr, link_path, gw, router_id } => {
            route_del(client, router_id, cidr, link_path, gw).await
        }
    }
}
