...
async fn test_multicast_list_and_get() {
    ...
    // Kind filtering selects one variant and excludes the other. An external
    // group's NAT target is an underlay group address, so it renders in the
    // DETAIL column of external rows; the KIND column is what distinguishes
    // the two, not the addresses appearing in a row.
