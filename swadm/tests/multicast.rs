...
// A second underlay group with no members just to test the empty-member
...
/// Run `swadm` with the given args and checks.
...
/// Collect the KIND column of every data row in a `multicast list` rendering,
/// skipping the header.
fn group_kinds(list: &str) -> Vec<String> {
    list.lines()
        .skip(1)
...
async fn test_multicast_list_and_get() {
    ...
    //
    // Both direction labels are exercised in the rendered output (in one go).
    ...
    assert!(
        list.contains(&format!("nat={NAT_IP}"))
            && list.contains("mac=11:22:33:44:55:66")
            && list.contains(&format!("vni={EXT_VNI}"))
            && list.contains("vlan=10")
            && list.contains(&format!("src={EXT_SOURCE}")),
        "list missing external forwarding detail:\n{list}"
    );
    // An SM group with an absent-VLAN and any-source branches.
    assert!(
    ...
    // Kind filtering selects one variant and excludes the other.
    let external_only = run_ok(&["multicast", "list", "-k", "external"]);
    let kinds = group_kinds(&external_only);
    assert!(
        !kinds.is_empty() && kinds.iter().all(|kind| kind == "external"),
        "external-only list is not filtered:\n{external_only}"
    );
    assert!(
        external_only.contains(&EXT_IPV4.to_string()),
        "external-only list missing external group:\n{external_only}"
    );

    let underlay_only = run_ok(&["multicast", "list", "--kind", "underlay"]);
    let kinds = group_kinds(&underlay_only);
    assert!(
        !kinds.is_empty() && kinds.iter().all(|kind| kind == "underlay"),
        "underlay-only list is not filtered:\n{underlay_only}"
    );
    assert!(
        underlay_only.contains(&UNDERLAY_IPV6.to_string()),
        "underlay-only list missing underlay group:\n{underlay_only}"
    );

    // The two filters compose.
    let both = run_ok(&["multicast", "list", "-t", TEST_TAG, "-k", "external"]);
    assert!(
        both.contains(&EXT_IPV4.to_string())
            && group_kinds(&both).iter().all(|kind| kind == "external"),
        "tag and kind filters do not compose:\n{both}"
    );

    // GET on the external group should show its detail, including
    // the aligned KV labels and the fully-populated NAT/VLAN/source values.
    ...
    // GET on the ASM group should show the "(none)"/"any" branches
    ...
    // GET on the underlay group should list both members, one per
    ...
    // GET on the member-less underlay group shows the "(none)" members branch.
