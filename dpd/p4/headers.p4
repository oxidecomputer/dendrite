...
@flexible
header bridge_md_h {
	bit<1>		nat_ingress_hit;
	bit<16>		body_checksum;
	bit<1>		icmp_recalc;
	bit<16>		icmp_csum;
	bit<16> 	l4_length;
	MulticastGroupId_t mcast_group;
	bit<8> 		flow_hash;
}
...
struct sidecar_ingress_meta_t {
	...
	bool is_mcast;                // packet is a multicast packet
	bool is_link_local_mcast;     // packet is link-local
